...
- **启动即闪退，终端报 `Error: No native build was found ... uiohook-napi`**：`uiohook-napi` 被主进程在启动时硬加载（全局输入钩子），但其 Linux 原生构建缺失。原因通常是构建机缺 X11 开发头（`libXt-devel`/`libXtst-devel`/`libXinerama-devel` 等，`X11/Intrinsic.h` 缺失），重建静默失败 → 先 `make deps` 装齐头文件再 `make build-app`。已将该模块列入关键模块并在构建后校验 `.node` 存在，缺失会直接报错而非交付坏包；已构建产物可 `make build-app` 重建或按报错修复。
