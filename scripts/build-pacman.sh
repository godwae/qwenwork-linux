...
pkgrel=1
pkgdesc='Unofficial local Linux conversion of QwenWorkCN (千问办公)'
arch=('$arch')
license=('MIT')
depends=('gtk3' 'nss' 'libxss' 'alsa-lib' 'libsecret' 'libxkbfile' 'xsel' 'libxtst')
source=()
sha256sums=()
...
SCRIPT
  ...
  if [ -f "$source_root/.qwenwork-linux/qwenwork-cn.png" ]; then
    install -m 0644 "$source_root/.qwenwork-linux/qwenwork-cn.png" "\$pkgdir/usr/share/icons/hicolor/256x256/apps/qwenwork-cn.png"
  fi
