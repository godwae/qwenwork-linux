...
# Build-time + run-time dependency installer for the QwenWorkCN Linux port.
#
# Besides the generic Electron/tooling packages, QwenWorkCN needs:
#   - libsecret dev headers : keytar rebuild (OS keychain)
#   - libXtst/libX11 headers: uiohook-napi rebuild (global input hooks)
#   - xsel                  : clipboardy's Linux backend (runtime dep)
#   - libxkbfile            : Electron keyboard layout handling (runtime dep)
...
main() {
    ...
    info "  put one official Intel/x64 QwenWorkCN DMG in downloads/"
    info "  make build-app"
    info "  make package"
    info "  make install"
}
