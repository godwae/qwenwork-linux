...
PACKAGE_NAME="${PACKAGE_NAME:-qwenwork-cn}"
PACKAGE_VERSION="${PACKAGE_VERSION:-$(date -u +%Y.%m.%d.%H%M%S)}"
RPM_VERSION="${PACKAGE_VERSION//+/_}"
RPM_VERSION="${RPM_VERSION//-/_}"
DESKTOP_TEMPLATE="$REPO_DIR/packaging/linux/qwenwork-cn.desktop"
...
EOF
    ...
    if [ -f "$APP_DIR/.qwenwork-linux/qwenwork-cn.png" ]; then
        cp "$APP_DIR/.qwenwork-linux/qwenwork-cn.png" \
            "$PKG_ROOT/usr/share/icons/hicolor/256x256/apps/qwenwork-cn.png"
    fi

    local icon_files_entry=""
    if [ -f "$APP_DIR/.qwenwork-linux/qwenwork-cn.png" ]; then
        icon_files_entry="/usr/share/icons/hicolor/256x256/apps/qwenwork-cn.png"
    fi

    cat > "$SPEC_FILE" <<EOF
# Exclude bundled multi-platform binaries from automatic dependency scanning.
# The app ships its own Electron runtime and vendored node native modules —
# their library deps are not resolvable via the system package manager.
...
Version: $RPM_VERSION
Release: 1%{?dist}
Summary: Unofficial local Linux conversion of QwenWorkCN (千问办公)
License: MIT
BuildArch: $arch
Requires: gtk3, nss, libXScrnSaver, alsa-lib, libsecret, libxkbfile, xsel, libXtst

%description
This package is generated locally from a user-owned official QwenWorkCN
macOS copy. It does not redistribute upstream software through the source
repository.
xsel is required by the bundled clipboardy module (main-process clipboard);
libXtst is required by uiohook-napi (global hotkeys).
